...
/// Remove a card from draw pile by index.
public fun remove_from_draw_pile(self: &mut Deck, index: u64): CardData {
    self.draw_pile.swap_remove(index)
}

/// Shuffle the draw pile using a provided RNG.
/// Uses Fisher-Yates shuffle.
public fun shuffle_draw_pile(self: &mut Deck, rng: &mut sui::random::RandomGenerator) {
    let len = self.draw_pile.length();
    if (len <= 1) { return };
    let mut i = len - 1;
    while (i > 0) {
        let j = rng.generate_u64_in_range(0, i);
